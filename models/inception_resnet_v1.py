...
class Block35(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.branch1[0](x)
        x1 = self.branch1[1](
            x1, kernel_size=self.kernel_size
        )  # Adjust kernel size dynamically

        x2 = self.branch2[0](x)
        x2 = self.branch2[1](
            x2, kernel_size=self.kernel_size
        )  # Adjust kernel size dynamically
        x2 = self.branch2[2](
            x2, kernel_size=self.kernel_size
        )  # Adjust kernel size dynamically
...
class Block17(nn.Module):
    ...
    def __init__(self, scale=1.0, kernel_size=7):
        ...
        self.branch1 = nn.Sequential(
            ...
            BasicConv2d(
                128, 128, kernel_size=(1, self.kernel_size), stride=1, padding=(0, 3)
            ),
            BasicConv2d(
                128, 128, kernel_size=(self.kernel_size, 1), stride=1, padding=(3, 0)
            ),
        )
...
class InceptionResnetV1(nn.Module):
    ...
    def __init__(
        ...
        self.elastic_config = (
            ...
            else self._sample_elastic_config()
        )
        ...
        self.repeat_1 = self._crate_sequential_from_config(
            "repeat_1", self.elastic_config
        )
        self.mixed_6a = Mixed_6a()
        self.repeat_2 = self._crate_sequential_from_config(
            "repeat_2", self.elastic_config
        )
        self.mixed_7a = Mixed_7a()
        self.repeat_3 = self._crate_sequential_from_config(
            "repeat_3", self.elastic_config
        )
